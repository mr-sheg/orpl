...
- `truncate` function in `calibration` module

## [1.0.8] - 2023-10-03

### Changed

- Headers of .txt and .csv files are checked to match requirements of ORPL.
  - Headers NEED to follow the structutre `xaxis,background,accumulation_0,accumulation_1,...` if header keys are missing or not named correctly, loading will fail.
  - Headers NEED at least 1 `accumulation_X` key to be valid
  - Header keys `xaxis` and `background` are optional

### Fixed

- Processing of spectrum from `.txt` and `.csv` files that don't have backgrounds
